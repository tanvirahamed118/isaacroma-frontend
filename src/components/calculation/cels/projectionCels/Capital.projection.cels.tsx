...
function CapitalProjectionCels({ data }: { data?: Business }) {
  const businessResults = data?.categories;

  const totalSalesRev = businessResults?.filter((item) => {
    return item.category === "CAPITAL_INCRIEASE_LOAN";
  });

  return (
    <React.Fragment>
      {totalSalesRev?.map((item) => {
        const { id, name, firstYear, secondYear, deviation, expectedPercent } =
          item || {};
        return (
...
export default CapitalProjectionCels;
