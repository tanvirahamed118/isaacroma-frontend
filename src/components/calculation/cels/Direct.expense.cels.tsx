...
function DirectExpenseCels({ data }: { data?: Business }) {
  const { t } = useTranslation();
  const businessResults = data?.businessResults;

  const directExpense = businessResults?.find((item) => {
    return item.name === "DIRECT_EXPENSES";
  });

  return (
    <React.Fragment>
      <tr className="bg-[#CCFFCC] font-bold">
        <td className="border border-black px-2 py-1">{t("direct_expense")}</td>
...
export default DirectExpenseCels;
