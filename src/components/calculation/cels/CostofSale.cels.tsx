...
function CostofSaleCels({ data }: { data?: Business }) {
  ...
  const totalSales = businessResults?.find((item) => {
    return item.name === "TOTAL_COST_OF_SALES";
  });
  ...
  return (
    <React.Fragment>
      <tr className="bg-[#CCFFCC] font-bold">
        <td className="border border-black px-2 py-1">{t("cost_of_sales")}</td>
