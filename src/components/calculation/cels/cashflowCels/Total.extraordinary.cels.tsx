...
function TotalExtraordinaryCels({ data }: { data?: Business }) {
  const businessResults = data?.businessResults;
  const { t } = useTranslation();
  const totalSalesRev = businessResults?.find((item) => {
    return item.name === "TOTAL_EXTRAORDINARY_EXPENSES";
  });
...
export default TotalExtraordinaryCels;
