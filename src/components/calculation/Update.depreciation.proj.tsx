...
function UpdateDepreciationProj({ selectId, isShow, setIsShow, name }: Types) {
  const { t } = useTranslation();
  const id = selectId;
  const { data } = useGetOneBusinessResultQuery(id);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    updateDepreciationProj({ category, id });
  };
  ...
  useEffect(() => {
    setCategory((prev) => {
      return {
        ...
        value:
          name === "secondYear"
            ? data?.business?.secondYear
            : name === "expectedPercent"
            ? data?.business?.expectedPercent
            : name === "deviation"
            ? data?.business?.deviation
            : "",
        userId: userId,
...
export default UpdateDepreciationProj;
