...
import BudgetSheetLoader from "../loader/Budget.sheet.loader";
...
function BudgetSheet() {
  ...
  const [getCashflowCalculation] = useGetCashflowCalculationMutation();
  const detail = { userId, businessId: id };
  const navigate = useNavigate();
  const { name, sector, description } = data?.business || {};
  const handleCalculation = () => {
    getCashflowCalculation(detail);
    navigate(`/create-business/cashflow/${id}`);
  };
  ...
  let content;
  if (isLoading) {
    content = <BudgetSheetLoader />;
  }
  if (!isLoading) {
    content = <BudgetTable business={data?.business} />;
  }
      ...
      <div className="p-3 md:p-5">
        <div className="w-full">
          ...
          <ul className="flex gap-2 items-center py-2">
            <li>
              <a href="" className="text-normal text-xm md:text-base">
                {t("dashboard")}
              </a>
            </li>
            <li>
        ...
        <div>
          ...
          <div className="flex gap-2 md:gap-5 md:flex-row flex-col item-center my-5">
            <Link to={`/create-business/${id}`} className="button text-center">
              {t("return_sto_summary")}
            </Link>
            <button className="button text-center" onClick={handleCalculation}>
              {t("go_to_cashflow")}
            </button>
            <button
