...
function UpdateCashflowPermont({ selectId, isShow, setIsShow }: Types) {
  const { t } = useTranslation();
  const id = selectId;
  const { data } = useGetCashflowPermonthQuery(id);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    updateCashflowPermonth({ category, id });
  };
...
export default UpdateCashflowPermont;
