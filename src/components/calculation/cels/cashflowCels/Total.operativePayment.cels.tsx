...
function TotalOperativePaymentCels({ data }: { data?: Business }) {
  const cashflows = data?.cashflows;
  const { t } = useTranslation();
  const prevousBalance = cashflows?.find((item) => {
    return item.name === "TOTAL_OPERATING_PAYMENTS";
  });
...
export default TotalOperativePaymentCels;
