...
function TotalPresentedColCels({ data }: { data?: Business }) {
  const cashflows = data?.cashflows;
  const { t } = useTranslation();
  const prevousBalance = cashflows?.find((item) => {
    return item.name === "TOTAL_PERCENT_FOR_COLLECTION";
  });
    ...
    <React.Fragment>
      <tr className="bg-[#CCFFCC] font-bold">
        <td
          ...
          colSpan={2}
        >
          {t("total_presented_for_collection")}
        </td>
...
export default TotalPresentedColCels;
