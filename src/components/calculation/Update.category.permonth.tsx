...
function UpdateCategoryPermonth({ selectId, isShow, setIsShow }: Types) {
  const { t } = useTranslation();
  const id = selectId;
  const { data } = useGetOnePermonthQuery(id);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    updateCategoryPermonth({ category, id });
  };
...
export default UpdateCategoryPermonth;
